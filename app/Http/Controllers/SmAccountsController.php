...
class SmAccountsController extends Controller
{
    ...
    public function fundTransferStore(Request $request){
        ...
        try{
            ...
            if($request->from_payment_method_name=="Bank" && $request->from_bank_name==''){
                    Toastr::error('Bank Name is Required', 'Error');
                    return redirect()->back();
            }elseif($request->to_payment_method_name=="Bank" && $request->to_bank_name==''){
                    Toastr::error('Bank Name is Required', 'Error');
                    return redirect()->back();
            }

            if($request->from_payment_method_name=="Bank" && $request->from_bank_name==$request->to_bank_name){
                $message=SmBankAccount::where('id',$request->from_bank_name)
                    ...
                    return redirect()->back();
            }elseif($request->from_payment_method_name==$request->to_payment_method_name){
                if($request->from_payment_method_name!="Bank"){
                    $message=SmPaymentMethhod::where('id',$request->from_payment_method_name)
                        ->where('school_id',Auth::user()->school_id)
                        ->first();
