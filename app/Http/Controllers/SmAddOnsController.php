...
class SmAddOnsController extends Controller
{
    ...
    public function moduleAddOnsEnable($name)
    {
        ...
        try {
            ...
            if ($name=='OnlineExam') {
                if ($check_enable_status) {
                    DB::update("UPDATE `infix_module_student_parent_infos` SET `route`='onlineexam/student-online-exam' WHERE `id`=46");
                    DB::update("UPDATE `infix_module_student_parent_infos` SET `route`='onlineexam/student_view_result' WHERE `id`=47");
                } else {
                    DB::update("UPDATE `infix_module_student_parent_infos` SET `route`='student-online-exam' WHERE `id`=46");
                    DB::update("UPDATE `infix_module_student_parent_infos` SET `route`='student_view_result' WHERE `id`=47");
