...
class SmStudentAdmissionController extends Controller
{
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        if ($parent) {
                            $user_info[] =  array('email' =>  $parent->guardians_email, 'id' => $parent->id, 'slug' => 'parent');
                        }
                            ...
                            if (count($user_info) != 0) {
                                ...
                                try{
                                    ...
                                    foreach($user_info as $info){
                                        $compact['data'] =  $info;                                          
                                    send_mail($info['email'], $info['name'], 'Login Credentials' , 'backEnd.studentInformation.user_credential',$compact);
                                 } 
                                }catch(\Exception $e){
                                    Log::info($e->getMessage());
                                }
    ...
    public function studentView(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_view', compact('student_detail', 'driver_info', 'fees_assigneds', 'fees_discounts', 'exams', 'documents', 'timelines', 'siblings', 'grades', 'academic_year', 'exam_terms','leave_details','optional_subject_setup','student_optional_subject','maxgpa','failgpaname'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
